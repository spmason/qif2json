    ...
    while(line = lines.shift()){
        ...
        switch(line[0]){
            ...
            case 'P':
                ...
                break;
            case 'L':
                var array = line.substring(1).split(":");
                transaction.category = array[0];
                if(array[1] !== undefined) {
                    transaction.subcategory = array[1];
                }
                break;
            case 'C':
                transaction.creation = line;
                break;
            case 'S':
                var array = line.substring(1).split(":");
                if(Object.keys(division).length) {
                ...
                division.category = array[0];
                if(array[1] !== undefined) {
                    division.subcategory = array[1];
                }
                break;
            case '$':
                division.amount = line.substring(1);
                break;
            default:
